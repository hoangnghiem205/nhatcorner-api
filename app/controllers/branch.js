module.exports = function (branch_model) {
    return {
        list: (req, res) => {
            if (req.params.page) {
            ...
            branch_model.findAndCountAll({ offset: (page - 1) * limit, limit: limit }).then((datas) => {
                res.json(datas || [])
            });
        },
        search: (req, res) => {
            ...
            branch_model.findAndCountAll({ offset: (page - 1) * limit, limit: limit, where: cond }).then((datas) => {
                res.json(datas || [])
            });
        },
        get: (req, res) => {
            const id = req.params.id;
            branch_model.findById(id).then((data) => {
        ...
        update: (req, res) => {
            ...
            branch_model.update(value, { where: { id: value.id } })
                .then((row) => {
                    if (row > 0) {
